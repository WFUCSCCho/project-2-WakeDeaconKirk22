...
public class Proj2 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        ArrayList<NBAPlayer> Player = new ArrayList<>();
        ArrayList<NBAPlayer> Backup=new ArrayList<>();
...
//Gives the rand insertion time
    long avlRandInsertTime = insertTime(avlTree, Player);
    long bstRandInsertTime = insertTime(mybst, Player);
    ...
    long  bstSortedSearchTime = searchTime(mybst,Player);
    long avlSortedSearchTime = searchTime(avlTree,Player);


        long avlSortedInsertTime = insertTime(avlTree,Player);;
        long bstSortedInsertTime = insertTime(mybst,Player);
...
String outFilePath = "./output.txt";
  ...
  writeToFile(result,outFilePath);
    ...
    private static long insertTime(BST<NBAPlayer> bst , ArrayList<NBAPlayer>NBAPlayer) {
        // Funnction call to count the insertion time for BST
        long time = System.nanoTime(); //start time

            for(int i = 0; i < NBAPlayer.size(); i++) {

                bst.insert(NBAPlayer.get(i));// inserts the player object
    ...
    private static long insertTime(AvlTree<NBAPlayer> AvlTree, ArrayList<NBAPlayer>NBAPlayer) {
        long time = System.nanoTime(); //start time
        // Funnction call to count the insertion time for AVL
        for(int i = 0; i < NBAPlayer.size(); i++) {

            AvlTree.insert(NBAPlayer.get(i)); // inserts the player object
        }
        long end = System.nanoTime(); // total time
        return end - time;
    }

private static long searchTime(AvlTree<NBAPlayer> AvlTree, ArrayList<NBAPlayer>NBAPlayer) {
    // Funnction call to count the search time for AVL
        long time = System.nanoTime(); //start time

        for(int i = 0; i < NBAPlayer.size(); i++) {

            AvlTree.contains(NBAPlayer.get(i));  // searches the player object
        }
    long end = System.nanoTime(); // total time
    return end - time;
}
private static long searchTime(BST<NBAPlayer>bst ,ArrayList<NBAPlayer>NBAPlayer) {
    // Funnction call to count the search time for BST
        long time = System.nanoTime(); //start time
        for(int i = 0; i < NBAPlayer.size(); i++) {

            bst.search(NBAPlayer.get(i));  // searches the player object
        }
    long end = System.nanoTime();
    return end - time; // total time
  }
